...
# 2018-11-24 Finished work on predict.py
...
def load_network(filepath, device):
    ...
    if 'idx2cat' in checkpoint:
        idx2cat = checkpoint['idx2cat']
    else:
        print('Warning! No index to category number dictionary found!', 
              '\nSubstituting with a one to one mapping.', '\nFLOWER NAMES ARE LIKELY TO BE WRONG!')
        idx2cat = dict()
        for dummy in range(0, 102):
            idx2cat[dummy] = str(dummy+1)    
    ...
    return my_model, my_optimizer, checkpoint['loss'], checkpoint['epoch'], my_labels, idx2cat
...
def main():
    ...
    model, optimizer, loss_tmp, epc_old, cat_to_name, idx2cat = load_network(
            in_args.network, in_args.device)
    if (cat_to_name == None):
    ...
    with torch.no_grad():
        ...
        catnum = idx2cat.get(myoutput.data.cpu().numpy().argmax()) 
        catname = cat_to_name.get(catnum)
    ...
    for cnt, sv in enumerate(myzip):
        ...
        top_classes.append(idx2cat.get(sv[1]))
        top_names.append(cat_to_name.get(top_classes[-1]))
        print(top_classes[-1], top_vals[-1], top_names[-1])
